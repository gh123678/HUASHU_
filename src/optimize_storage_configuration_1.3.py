# ============================================================================
# 注意：本脚本为队友初版实现（递进三层分析框架第三步），保留作为工作记录。
#
# 与统一 LP 方案（model.py + solve.py）的差异：
#   1. 弃电分摊：本脚本按风/光发电比例平摊弃电量，
#      统一方案约定 C 园区"优先弃风电"（风电 0.5 元 > 光伏 0.4 元）。
#   2. 求解器：本脚本使用 scipy.optimize.milp，统一方案使用 linprog(method='highs')。
#   3. 扫描精度：本脚本步长固定 10kW/10kWh，统一方案粗扫后加密至 5kW/10kWh。
#   4. 最优储能配置 (P_opt, E_opt) 与统一方案一致，可作为交叉验证引用。
# ============================================================================
